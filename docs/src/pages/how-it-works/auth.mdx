...
Tokens are sent from the API in the response body. When the request is made from SvelteKit server side,
...
On the UI, each API request is intercepted and modified using the server `hook`:
