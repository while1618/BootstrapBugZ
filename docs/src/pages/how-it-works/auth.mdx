...
Both tokens are sent to the user in the response body and consumed by the UI. First the request will be made to the API from SvelteKit server side.
Both access and refresh tokens will be saved in HTTP-only cookie and they will be used to keep the user authenticated.

#### Token Storage and Lifetime

- **Access Token**: Not stored in a database on the backend and it is valid for 15 minutes.
- **Refresh Token**: Stored in Redis on the backend and valid for 7 days.

You can change expiration times through environment variables.

#### Refreshing Token

On the UI, each API request is intercepted and modified using the `hook`:
...
- Tokens are removed from HTTP-only cookies on the UI.
...
- The user's ID and the time of the request are stored in Redis. This is used to ensure that all access tokens created before the sign-out request are invalid.
