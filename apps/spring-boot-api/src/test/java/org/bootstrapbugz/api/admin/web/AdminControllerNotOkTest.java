...
class AdminControllerNotOkTest {
  ...
  void loginAdmin() throws Exception {
    ...
    adminToken = resultActions.andReturn().getResponse().getHeader(AuthUtil.AUTH_HEADER);
  }
    ...
    ResultActions resultActions =
        mockMvc.perform(
            post(Path.AUTH + "/login")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(loginRequest)));
    userToken = resultActions.andReturn().getResponse().getHeader(AuthUtil.AUTH_HEADER);
  }
    ...
    mockMvc
        .perform(
            get(Path.ADMIN + "/users")
                .contentType(MediaType.APPLICATION_JSON)
                .header(AuthUtil.AUTH_HEADER, userToken))
    ...
    mockMvc
        .perform(
            put(Path.ADMIN + "/users/role")
                .contentType(MediaType.APPLICATION_JSON)
                .header(AuthUtil.AUTH_HEADER, adminToken))
