...
class AuthServiceImpl(
    ...
    override fun register(registerUserRequest: RegisterUserRequest): UserDTO {
        if (userRepository.existsByUsername(registerUserRequest.username)) throw ConflictException(
            messageService.getMessage("username.exists")
        )
        if (userRepository.existsByEmail(registerUserRequest.email)) throw ConflictException(
            messageService.getMessage("email.exists")
        )
    ...
    private fun createUser(registerUserRequest: RegisterUserRequest): User {
        val roles = roleRepository.findByName(RoleName.USER)
            .orElseThrow { RuntimeException(messageService.getMessage("role.notFound"))}
        return User(
    ...
    override fun resetPassword(resetPasswordRequest: ResetPasswordRequest) {
        val user = userRepository
            ...
            .orElseThrow { BadRequestException("token", messageService.getMessage("token.invalid")) }
    ...
    override fun sendVerificationMail(request: VerificationEmailRequest) {
        val user = userRepository
            ...
            .orElseThrow { ResourceNotFoundException(messageService.getMessage("user.notFound")) }
        if (Boolean.TRUE == user.active) throw ConflictException(
            messageService.getMessage("user.active")
        )
        val token = verificationTokenService.create(user.id)
        verificationTokenService.sendToEmail(user, token)
    }
