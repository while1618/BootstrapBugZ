...
class AuthController(private val authService: AuthService) {
    ...
    fun register(@Valid @RequestBody registerUserRequest: RegisterUserRequest): ResponseEntity<UserDTO> {
        return ResponseEntity(authService.register(registerUserRequest), HttpStatus.CREATED)
    }

    @PostMapping("/tokens")
    fun authenticate(@Valid @RequestBody authTokensRequest: AuthTokensRequest, request: HttpServletRequest): ResponseEntity<AuthTokensDTO> {
    ...
    fun refreshTokens(@Valid @RequestBody refreshAuthTokensRequest: RefreshAuthTokensRequest, request: HttpServletRequest): ResponseEntity<AuthTokensDTO> {
    ...
    fun sendVerificationMail(@Valid @RequestBody verificationEmailRequest: VerificationEmailRequest): ResponseEntity<Void> {
        authService.sendVerificationMail(verificationEmailRequest)
        return ResponseEntity.noContent().build()
    }

    @PostMapping("/verify-email")
    fun verifyEmail(@Valid @RequestBody verifyEmailRequest: VerifyEmailRequest): ResponseEntity<Void> {
