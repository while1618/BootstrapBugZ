...
@ControllerAdvice
class CustomExceptionHandler(private val messageService: MessageService) : ResponseEntityExceptionHandler() {
    override fun handleMethodArgumentNotValid(
        ...
        result
            .fieldErrors
            .forEach(Consumer { error: FieldError -> errorMessage.addDetails(error.field, error.defaultMessage) })
        result.globalErrors.forEach(Consumer { error: ObjectError -> errorMessage.addDetails(error.defaultMessage) })
        return ResponseEntity(errorMessage, setHeaders(), status)
    }

    private fun createError(status: HttpStatus, message: String?, field: String?): ResponseEntity<Any> {
        val errorMessage = ErrorMessage(status)
        if (field != null) errorMessage.addDetails(field, message!!) else errorMessage.addDetails(message!!)
        return ResponseEntity(errorMessage, setHeaders(), status)
    }
